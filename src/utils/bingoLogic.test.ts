...
import { FREE_SPACE, getQuestionsForSet, questions } from '../data/questions'
...
describe('bingoLogic', () => {
  describe('generateBoard', () => {
    ...
    it('should use the provided question set', () => {
      const socialQuestions = getQuestionsForSet('social')
      const board = generateBoard(socialQuestions)
      const texts = board.filter((square) => !square.isFreeSpace).map((square) => square.text)

      texts.forEach((text) => {
        expect(socialQuestions).toContain(text)
      })
    })
  })
